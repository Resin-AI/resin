...
For Codex CLI 0.156.1 rollouts, the local observer decodes native `session_meta`, `response_item`, and `event_msg` records for session lifecycle, model context, messages, tool calls/results, and reported token usage. A command completion is recorded only from an explicit command-end status or native command-output metadata before `Final output:`; text printed by a command cannot manufacture success. Missing status and usage are not treated as zero or success. Unsupported records remain available as unknown events rather than inferred activity.
