...
For Codex CLI 0.156.1 rollouts, the local observer decodes native `session_meta`, `response_item`, and `event_msg` records for session lifecycle, model context, messages, tool calls/results, and reported token usage. Code-mode shell activity comes from structured `item_completed` / `CommandExecution` items, not from interpreting the `exec` wrapper as shell source. These items require a terminal status and explicit exit code; argv, working directory, output and native duration are retained. Command-output status is trusted only before `Final output:`; text printed by a command cannot manufacture success. Missing status and usage are not treated as zero or success. Unsupported records remain available as unknown events rather than inferred activity.
