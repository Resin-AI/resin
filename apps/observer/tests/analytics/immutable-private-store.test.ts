...
import { FilePrivateValueStore, InMemoryPrivateValueStore, resolvePrivateReference } from "../../src/analytics/private-value-store.js";
...
describe("immutable private reference persistence", () => {
  it("does not overwrite a value or its owner, including through another store instance", () => {
    ...
    expect(() => second.set(key, "replacement", origin, "literal")).toThrow("different value or owner");
    expect(() => second.set(key, original, { workspaceId: "ws-foreign" }, "literal")).toThrow("different value or owner");
    ...
    expect(() => memory.set(key, "replacement", origin, "literal")).toThrow("different value or owner");
  });
  ...
  it("retains all entries written by separate simultaneous processes", async () => {
    ...
    await Promise.all(["a", "b", "c"].map((writer) => new Promise<void>((resolve, reject) => {
      const child = spawn(process.execPath, ["--input-type=module", "-e", source, root, writer], { stdio: ["ignore", "ignore", "pipe"] });
      let diagnostic = "";
    ...
    for (const writer of ["a", "b", "c"]) {
      for (let i = 0; i < 64; i++) expect(reopened.get(`private:v2:${writer}:${i}`)).toEqual({ writer, i });
    }
    const entries = path.join(root, "private-values", "entries-v2");
    expect(readdirSync(entries)).toHaveLength(192);
    if (process.platform !== "win32") {
      for (const name of readdirSync(entries)) expect(statSync(path.join(entries, name)).mode & 0o777).toBe(0o600);
    }
  });
});
