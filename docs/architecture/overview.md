...
- Ordinary non-program JSON data leaves may produce typed, unconfirmed input proposals without exposing values or defaults. Prior-result proposals take precedence at the same position. Independent recorded variations must distinguish the proposed binding from its original value before it becomes an executable input or result edge.
