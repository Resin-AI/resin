...
import { FilePrivateValueStore, resolvePrivateReference } from "../../src/analytics/private-value-store.js";
import { WorkflowCallRecorder, readWorkflowCallCarrier } from "../../src/analytics/workflow-call-recorder.js";
...
async function capture(store: FilePrivateValueStore, sessionId: string, recorder = new WorkflowCallRecorder({ privateValues: store })) {
  const pipeline = new NormalizationPipeline({
    privateValueStore: store,
    redactionConfig: { customSecrets: [secret], maxStringLength: 32, homeDir: "/home/source", repoRoot: "/home/source/project" },
  });
  const parameters = [
  ...
  const answers = [`order_id,product\n${secret}\n${sessionId}-first,widget\n`, `order_id,product\n${secret}\n${sessionId}-second,gizmo\n`];
  const rows = [
    { type: "tool_call", callId: "first", toolName: "read", parameters: parameters[0] },
    { type: "tool_result", callId: "first", toolName: "read", result: answers[0], isError: false, executionDurationMs: 1 },
    ...
    { type: "tool_result", callId: "second", toolName: "read", result: answers[1], isError: false, executionDurationMs: 1 },
  ];
  ...
  for (const [index, row] of rows.entries()) {
    const result = await pipeline.processIntermediateEvent({
      ...row, sessionId, timestamp: "2026-09-19T00:00:00.000Z",
...
describe("exact local values across the real normalization boundary", () => {
  it("reconstructs final demonstration values before masks, aliases, and truncation", async () => {
    ...
    const recipe = recordCallsFromEvents("wf-recovery", captured.events.slice(0, 2), { supportingEvents: captured.events.slice(2) })!;
  ...
  it("does not reinterpret original text that merely looks like a redaction marker", async () => {
    ...
    const pipeline = new NormalizationPipeline({ redactionConfig: { scanContent: false, sensitiveEnvVars: [] } });
    const recorder = new WorkflowCallRecorder({ privateValues: store });
    const original = { nested: ["literal [REDACTED_SECRET:example]", 7, false, null], payload: { value: "ordinary JSON", reference: "also data" } };
    const result = await pipeline.processIntermediateEvent({
      sessionId: "session-literal", type: "tool_call", toolName: "unfamiliar", callId: "call-literal", parameters: original,
      timestamp: "2026-09-19T00:00:00.000Z", causalRef: { causalSequence: 1, parentId: null },
    }, origin);
    if (result.status !== "success") throw new Error(result.errorReason);
    ...
    if (payload.type !== "object") throw new Error("Native object was interpreted as a Resin envelope");
  ...
  it("keeps references stable across sessions, fresh instances, and clear/restart", async () => {
    ...
    const recipe = recordCallsFromEvents("wf-a", first.events.slice(0, 2), { supportingEvents: first.events.slice(2) })!;
    const input = recipe.workflow.heldOut!.inputs.find((entry) => entry.argument === "path")!;
    ...
    expect(replayed.events.map((entry) => entry.metadata?.workflowCall ?? entry.metadata?.workflowResult)).toEqual(
